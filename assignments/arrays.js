...
let sort = {
  sorting : function(){
      ...
      for (i=0; i< inventory.length; i++){
          let y= inventory[i].car_year;
          this.carYears[i] = y;
      }
  }
}
sort.sorting();
console.log(sort.carYears)
...
const oldCars = inventory.filter(function(item){
  return item.car_year>2000;
})
console.log(oldCars)
...
const BMWCars = inventory.filter(function(item){
  return item.car_make===`BMW`;
})
const AudiCar = inventory.filter(function(item){
  return item.car_make===`Audi`;
})
let BMWAndAudi=[BMWCars, AudiCar]
